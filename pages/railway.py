...
def render_railway():
    ...
    with tab1:
        ...
        if st.button("Check PNR", key="pnr_check", use_container_width=True, help="Check PNR status"):
    ...
    with tab2:
        ...
        for i in range(5):
            if get_date_label(i) in date_choice:
                offset = i
                break
        if st.button("Get Live Status", key="train_live", use_container_width=True, help="Get real-time train status"):
    ...
    with tab3:
        ...
        if st.button("Get Schedule", key="train_sch", use_container_width=True, help="Get complete train schedule"):
        ...
        if 'sch_data' in st.session_state:
            ...
            with col1:
                if start > 0:
                    if st.button("◀ Previous", key="sch_prev", help="Previous page"):
            ...
            with col3:
                if end < total:
                    if st.button("Next ▶", key="sch_next", help="Next page"):
