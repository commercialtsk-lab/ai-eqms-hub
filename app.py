...
5. Multiple entries: If a table has multiple rows, extract ALL valid entries (max 5).
...
# ========== PDF PRINT (A4 filtered) ==========
def generate_print_pdf(df, sheet_name):
    pdf = FPDF(orientation='L', unit='mm', format='A4')
    pdf.add_page()
    pdf.set_font("Arial", "B", 11)
    pdf.cell(0, 8, f"AI EQMS Hub Pro - {sheet_name} (Filtered Print)", ln=True, align="C")
    pdf.set_font("Arial", "", 8)
    pdf.cell(0, 6, f"Generated: {now_ist().strftime('%d-%m-%Y %H:%M:%S IST')} | Rows: {len(df)}", ln=True, align="C")
    pdf.ln(4)

    # Select useful columns (limit for A4 landscape)
    cols = list(df.columns)[:12]
    col_width = 277 / max(len(cols), 1)
    ...
    for _, row in df.iterrows():
        for col in cols:
            ...
            pdf.cell(col_width, 5, val, border=1)
        pdf.ln()
        if pdf.get_y() > 190:
            pdf.add_page()
            pdf.set_font("Arial", "B", 7)
...
def main():
    ...
    # ---- SIDEBAR ----
    with st.sidebar:
        # Greeting with flag colours + Hindi Devanagari
        now = now_ist()
        hour = now.hour
        if is_flag_time():
            st.markdown("""
            <div style="text-align:center; margin-bottom:12px; line-height:1.5;">
                ...
                <div style="font-weight:700; color:#FF9933; font-size:1.05rem;">नमस्ते, आपका स्वागत है</div>
                <div style="font-weight:600; color:#000000; font-size:0.95rem;">हम भारत के लोग</div>
                <div style="font-weight:700; color:#138808; font-size:1.0rem;">जय हिंद</div>
            </div>
        ...
        # ---- Auto Refresh ----
        auto_refresh = st.checkbox("🔄 Auto Sync (20s)", value=True)
        if auto_refresh:
        ...
        upload_type = st.selectbox(
            ...
            help="PDF / Image ideally 1-2 pages only"
        )
        ...
        if st.button("🚀 Process & Save", use_container_width=True, type="primary"):
            if uploaded_file:
                ...
                if upload_type != "Auto Detect":
                    mapping = {"Image": "image", "PDF": "pdf", "Text": "text", "Audio": "audio"}
                    file_type = mapping.get(upload_type, file_type)
        ...
        if st.session_state.upload_success and st.session_state.last_uploaded_file:
            ...
            with c1:
                if st.session_state.last_uploaded_view_url:
                    st.link_button("👁️ View File", st.session_state.last_uploaded_view_url, use_container_width=True)
            with c2:
                if st.session_state.last_uploaded_drive_url:
                    st.link_button("🖨️ Print File", st.session_state.last_uploaded_drive_url, use_container_width=True)
        ...
        c1, c2 = st.columns(2)
        with c1:
            from_input = st.date_input("📅 From DOJ (DD-MM-YYYY)", value=st.session_state.from_val, key="from_input_widget", format="DD-MM-YYYY")
        with c2:
            to_input = st.date_input("📅 To DOJ (DD-MM-YYYY)", value=st.session_state.to_val, key="to_input_widget", format="DD-MM-YYYY")

        # Immediate filter application
        if (pnr_input != st.session_state.pnr_val or
            train_input != st.session_state.train_val or
            from_input != st.session_state.from_val or
            to_input != st.session_state.to_val):
        ...
        # ---- Navigation ----
        view = st.radio("👁️ View", ["📋 Data Table", "📊 Dashboard", "💬 Chat with Gemini"], index=0)

    # ---- MAIN AREA ----
    st.markdown("<h1 style='font-size:1.55rem; font-weight:700; margin-bottom:2px;'>🚂 AI EQMS Hub Pro</h1>", unsafe_allow_html=True)
    st.markdown(f"<p style='color:#6b7280; font-size:0.9rem; margin-bottom:10px;'>Enterprise Railway EQ Management System • Last sync: {now_ist().strftime('%H:%M:%S')} IST</p>", unsafe_allow_html=True)
    ...
    if view == "💬 Chat with Gemini":
        ...
        if not filtered_df.empty:
            ...
            if train_col and train_col in filtered_df and filtered_df[train_col].notna().any():
                ...
                fig_pie.update_layout(height=360, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
                fig_pie.update_traces(textposition='inside', textinfo='percent+label')
                st.plotly_chart(fig_pie, use_container_width=True)

            # Histogram (box style) below pie
            berth_col = next((c for c in filtered_df.columns if 'BERTH' in c.upper() or 'T/BERTHS' in c.upper()), None)
            if berth_col and berth_col in filtered_df:
                ...
                if not berth_vals.empty:
                    ...
                    fig_hist.update_layout(height=320, bargap=0.15, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            ...
            if doj_col and doj_col in filtered_df:
                ...
                if not daily_counts.empty:
                    ...
                    fig_line.update_layout(height=320, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        ...
        if filtered_df.empty:
            ...
            with col_print1:
                if st.button("🖨️ Print Filtered (A4)", use_container_width=True):
                    try:
                        pdf_bytes = generate_print_pdf(filtered_df, sheet_choice)
                        st.download_button(
                            label="⬇️ Download A4 PDF",
                            data=pdf_bytes,
                            file_name=f"EQMS_{sheet_choice}_filtered_{now_ist().strftime('%d%m%Y_%H%M')}.pdf",
                            mime="application/pdf",
                            use_container_width=True
                        )
                        st.toast("✅ PDF ready for print!", icon="🖨️")
                    except Exception as e:
            ...
            if not page_df.empty:
                ...
                with b4:
                    if selected_indices:
                        ...
                        st.button("📤 WhatsApp Share", disabled=True, use_container_width=True)
                ...
                # Quick share note
                st.caption("💡 Tip: Select rows → use WhatsApp Share. On mobile you can also use system share.")
