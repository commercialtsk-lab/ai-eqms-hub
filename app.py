...
def main():
    ...
    with st.sidebar:
        ...
        # Print Sheet link in sidebar
        st.markdown("---")
        st.markdown("### 🖨️ Print Sheet")
        print_key = f'print_b64_{sheet_choice}'
        print_b64_val = st.session_state.get(print_key, '')
        if print_b64_val:
            print_data_url = f"data:text/html;base64,{print_b64_val}"
            st.link_button("🖨️ Open & Print", print_data_url, use_container_width=True, type="secondary")
            st.caption("👆 Click karein, naya tab mein print hoga")
        else:
            st.info("📋 Data Table view mein jayein, print link yahan ayega")
