...
def init_drive():
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_dict(
        creds_dict, ["https://www.googleapis.com/auth/drive.file"]
    )
...
Return ONLY valid JSON array. No extra text."""
    ...
    try:
        ...
        for r in records:
            ...
            r["PNR"] = pnr
            if r.get("PASS_PH"):
                r["PASS_PH"] = clean_phone(r["PASS_PH"])
            if r.get("DOJ"):
                r["DOJ"] = parse_date(r["DOJ"])
...
def apply_theme(dark):
    if dark:
        bg, card, text, sec, border, accent, inp = (
            "#0a0a0a",
            "#141414",
            "#e8e8e8",
            "#a0a0a0",
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown(
            f'<a href="https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit" target="_blank" style="display:block;text-align:center;padding:9px;border:1px solid #444;border-radius:10px;text-decoration:none;color:#a855f7;font-weight:500;">📊 Open Google Sheet</a>',
            unsafe_allow_html=True,
        )
        st.markdown("---")
        ...
        if mode == "📷 Image / PDF":
            ...
            text_data = st.text_area(
                "Paste messy text",
                height=130,
                placeholder="Yahan text paste / type karein...",
                label_visibility="collapsed",
            )
        else:
        ...
        if (
            pnr != st.session_state.pnr_val
            or train != st.session_state.train_val
            or fr != st.session_state.from_val
            or to != st.session_state.to_val
        ):
    ...
    # ---------- MAIN AREA ----------
    st.markdown(
        "<h1 style='font-size:1.55rem;margin-bottom:2px;'>🚂 AI EQMS Hub Pro</h1>",
        unsafe_allow_html=True,
    )
    ...
    if not filtered.empty:
        if st.session_state.pnr_val and cfg.get("pnr_col") is not None:
            ...
            filtered = filtered[
                filtered[col].astype(str).str.contains(st.session_state.pnr_val, case=False, na=False)
            ]
        if st.session_state.train_val and cfg.get("train_col") is not None:
            col = filtered.columns[cfg["train_col"]]
            filtered = filtered[
                filtered[col].astype(str).str.contains(st.session_state.train_val, case=False, na=False)
            ]
    ...
    # ==================== DATA TABLE ====================
        ...
        if not filtered.empty:
            try:
                ...
                st.warning(f"PDF generate nahi ho paya: {e}")
        ...
        if filtered.empty:
            ...
            with b4:
                if selected:
                    ...
                    st.link_button(
                        "📤 WhatsApp",
                        f"https://api.whatsapp.com/send?text={urllib.parse.quote(msg)}",
                        use_container_width=True,
                    )
                else:
    ...
    # ==================== DASHBOARD ====================
        ...
        if dcol:
            for _, r in filtered.iterrows():
                try:
                    if datetime.strptime(parse_date(r.get(dcol, "")), "%d-%m-%Y") < now_ist().replace(
                        tzinfo=None
    ...
    # ===== FOOTER =====
    st.markdown(
        '<div class="pro-footer">Made with ❤️ by Sharique • AI EQMS Hub Pro</div>',
        unsafe_allow_html=True,
    )
