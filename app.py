...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Current sheet ko print karein")
        if st.button("🖨️ PRINT Sheet", use_container_width=True, key="sidebar_print_btn", type="secondary"):
            st.session_state.print_trigger_sheet = sheet_choice
            st.rerun()
    ...
    # ------------------------------------------------------------------
        ...
        if filtered_df.empty:
            ...
            # Handle print trigger from sidebar
            if st.session_state.get('print_trigger_sheet') == sheet_choice and not print_df_full.empty:
                html_table = print_df_full.to_html(index=False, border=1, classes='print-table', justify='center')
                print_html = f"""
                <!DOCTYPE html>
                <html>
                <head><title>{sheet_choice} Sheet - AI EQMS Hub Pro</title>
                <style>
                ...
                # Use a hidden component to trigger popup
                popup_js = f"""
                <script>
                (function(){{
                    var html = atob('{print_b64}');
                ...
                st.session_state.print_trigger_sheet = None
                st.toast("🖨️ Print window opened!", icon="🖨️")
                    ...
                    if not train_no or not train_no.isdigit() or not (3 <= len(train_no) <= 5):
                        ...
                        with st.spinner("Fetching live status..."):
                            ...
                            if data and isinstance(data, dict) and data.get('error'):
                                ...
                                st.error(f"❌ Error: {err_msg}" + (f" | {detail}" if detail else ""))
                                with st.expander("🔍 Raw Response"):
                                    st.json(data)
                            elif data:
                                result_text = format_live_train_result(data)
