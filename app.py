...
def make_pdf(df, title):
    ...
    return bytes(output) if isinstance(output, (bytearray, memoryview)) else output
...
def main():
                ...
                try:
                    ...
                    if "error" in res:
                        ...
                        if "error" in save_res:
                            ...
                            st.success(f"Saved {save_res['saved']}")
    ...
    if view == "💬 Chat":
        ...
        cols = st.columns(3)
        for i, s in enumerate(["Show EQ summary", "Records today?", "Train breakup", "Pending EQ", "Quota status", "PNR help"]):
    ...
    # ========== DATA TABLE ==========
        ...
        if not filtered.empty:
            try:
                ...
                with colp1:
                    st.download_button(
                        "🖨️ Download A4 PDF (Print ke liye)",
                        data=pdf_bytes,
        ...
        if filtered.empty:
            ...
            with b4:
                if selected:
                    ...
                    st.link_button("📤 WA", f"https://api.whatsapp.com/send?text={urllib.parse.quote(msg)}", use_container_width=True)
                else:
                    st.button("📤 WA", disabled=True, use_container_width=True)
